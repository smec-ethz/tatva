...
from tatva.compound import Compound, field, FieldType, FieldSize
from tatva.mpi import PartitionInfo, ExchangePlan
...
def test_exchange_plan_layout():
    ...
    if rank == 0:
        ...
        # s: [2]
        # v: [3]
        np.testing.assert_array_equal(plan.local_to_global_dofs, [0, 1, 4, 5, 2, 3])
        np.testing.assert_array_equal(plan.owned_mask, [True, True, False, False, True, True])
        ...
        # s: [2]
        # v: [6]
        np.testing.assert_array_equal(plan.local_to_global_dofs, [4, 5, 0, 1, 2, 6])
        np.testing.assert_array_equal(plan.owned_mask, [True, True, False, False, False, True])
...
def test_exchange_plan_communication():
    ...
    if rank == 0:
        p_info = PartitionInfo(nodes_local_to_global=np.array([0, 1], dtype=np.int32), n_owned_nodes=1)
    elif rank == 1:
        p_info = PartitionInfo(nodes_local_to_global=np.array([1, 0], dtype=np.int32), n_owned_nodes=1)
    else:
        p_info = PartitionInfo(nodes_local_to_global=np.array([], dtype=np.int32), n_owned_nodes=0)
...
def test_exchange_plan_incomplete_nodal():
    ...
    if rank == 0:
        ...
        p_info = PartitionInfo(nodes_local_to_global=np.array([1, 2], dtype=np.int32), n_owned_nodes=2)
    ...
    mock_mesh = Mesh(coords=jnp.zeros((len(p_info.nodes_local_to_global), 1)), elements=jnp.zeros((0, 2), dtype=int))

    class MyState(Compound, mesh=mock_mesh):
        u = field(shape=(FieldSize.AUTO, 1), field_type=FieldType.NODAL)
        l = field(shape=(len(subset), 1), field_type=FieldType.NODAL, nodal_local_to_global=subset)
    ...
    if rank == 0:
        ...
        np.testing.assert_array_equal(plan.local_to_global_dofs, [0, 2, 1])
        np.testing.assert_array_equal(plan.owned_mask, [True, False, True])
    elif rank == 1:
        assert plan.local_size == 3
        # u local: [node 1 (owned), node 2 (owned)]. Global IDs: [2, 3]
        # l local: [node 2 (owned)]. Global IDs: [4]
        np.testing.assert_array_equal(plan.local_to_global_dofs, [2, 3, 4])
        np.testing.assert_array_equal(plan.owned_mask, [True, True, True])
