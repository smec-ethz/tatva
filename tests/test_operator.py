...
def _manual_map_results(func, dof_values: jnp.ndarray) -> np.ndarray:
    ...
    for element in np.array(ELEMENTS):
        element_values = np.array(dof_values)[element]
        element_results = [
...
def test_map_respects_element_quantity(tri_operator: Operator):
    ...
    result = mapped(dof_values, element_bias)
...
def test_map_over_elements_matches_manual(tri_operator: Operator):
    ...
    expected = np.array(
        [np.sum(np.array(dof_values)[element]) for element in np.array(ELEMENTS)]
    )
    np.testing.assert_allclose(result, expected)


def test_eval_returns_weighted_average(tri_operator: Operator):
    dof_values = jnp.array([0.0, 1.0, 2.0, 3.0], dtype=jnp.float64)
    result = tri_operator.eval(dof_values)
...
def test_grad_of_linear_field_is_constant(tri_operator: Operator):
    ...
    dof_values = NODES @ coeffs
    grad = tri_operator.grad(dof_values)
...
def test_integrate_nodal_field_matches_area(tri_operator: Operator):
    dof_values = jnp.ones(NODES.shape[0], dtype=jnp.float64)
    per_element = tri_operator.integrate_per_element(dof_values)
    total = tri_operator.integrate(dof_values)
...
def test_interpolate_recovers_linear_function(tri_operator: Operator):
    dof_values = jnp.sum(NODES, axis=1)
    points = jnp.array(
        [
            [0.25, 0.25],
            [0.75, 0.25],
    ...
    interpolated = tri_operator.interpolate(dof_values, points)
...
def test_interpolate_raises_for_points_outside_mesh(tri_operator: Operator):
    ...
    with pytest.raises(RuntimeError):
        tri_operator.interpolate(dof_values, points)
...
def test_operator_can_be_used_as_dynamic_jit_argument(tri_operator: Operator):
    ...
    result = integrated_area(tri_operator, dof_values)
