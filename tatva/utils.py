...
def make_gradient_trace(
    ...
    Useful for gradient-based averaging at interfaces such as flux averaging, stress averaging.

    Args:
        mesh: The bulk mesh containing the full domain.
        trace_elements: Array of shape (n_trace_elements, nodes_per_trace_element).
        elem_type: The element type used in the bulk mesh (e.g., Tri3).
        trace_elem_type: The element type used in the trace mesh (e.g., Line2).
    """
    ...
    sub_qp_xi = jnp.einsum("qa,eac->eqc", N_sub, face_ref_nodes).reshape(-1, bulk_ref_nodes.shape[1])
