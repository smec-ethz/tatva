...
from typing import Callable, Generic, ParamSpec, Protocol, TypeAlias, TypeVar, cast, Tuple, Optional, Any
...
class Operator(Generic[ElementT], eqx.Module):
    ...
    _nse: Optional[int] #= eqx.field(default=None, static=True)
    _indices_T: Optional[Array] #= eqx.field(default=None, static=True)
    ...
    def __check_init__(self) -> None:
        ...
        if coords.ndim != 2:
            raise ValueError("Mesh coordinates must be a 2D array shaped (n_nodes, n_dim).")
        if coords.shape[0] == 0:
            raise ValueError("Mesh must contain at least one node.")

        if elements.ndim != 2:
            raise ValueError("Mesh elements must be a 2D array shaped (n_elements, n_nodes_per_element).")
        if elements.shape[0] == 0:
            raise ValueError("Mesh must contain at least one element.")
        if not jnp.issubdtype(elements.dtype, jnp.integer):
            raise TypeError("Mesh element connectivity must contain integer indices.")
        ...
        if quad_points.ndim != 2 or quad_points.shape[0] == 0:
            raise ValueError("Element must define at least one quadrature point in an (n_q, n_dim) array.")

        local_dim = quad_points.shape[1]
        global_dim = coords.shape[1]
        if local_dim > 1 and local_dim != global_dim:
            raise ValueError(f"Element {self.element.__class__.__name__} expects {local_dim}D coordinates but mesh provides {global_dim}D nodes.")
        
        n_nodes_per_element = elements.shape[1]
        # Check shape function on first quad point
        shape_fn = np.asarray(self.element.shape_function(self.element.quad_points[0]))
        if shape_fn.shape[0] != n_nodes_per_element:
            raise ValueError(f"Mesh connectivity lists {n_nodes_per_element} nodes per element but {self.element.__class__.__name__} expects {shape_fn.shape[0]}.")
    ...
    def integrate(self, arg: jax.Array | Numeric) -> jax.Array:
        ...
        if ctx.active_op is self:
            ...
            det_J = jax.vmap(lambda xi: self.element.get_jacobian(xi, ctx.el_coords)[1])(self.element.quad_points)
    ...
    def eval(self, nodal_values: jax.Array) -> jax.Array:
        ...
        if ctx.active_op is self:
            ...
            return jax.vmap(lambda xi: self.element.interpolate(xi, nodal_values))(self.element.quad_points)
        ...
        if self.mesh is ctx.active_op.mesh:
            ...
            el_u_flat = nodal_values.reshape(n_nodes, -1)[self.mesh.elements[ctx.el_idx]]
            ...
            return jax.vmap(lambda xi: self.element.interpolate(xi, el_u))(ctx.active_op.element.quad_points)
    ...
    def grad(self, nodal_values: jax.Array) -> jax.Array:
        ...
        if ctx.active_op is self:
            # nodal_values is the LOCAL element vector here
            return jax.vmap(lambda xi: self.element.gradient(xi, nodal_values, ctx.el_coords))(self.element.quad_points)
    ...
    def get_element_assembler(self, energy_wrapper_fn: Callable) -> Callable:
        ...
        def assemble_loop(u_flat):
            ...
            # Gather
            el_u = u_flat.reshape(n_nodes, -1)[self.mesh.elements].reshape(self.mesh.elements.shape[0], -1)
            el_x = self.mesh.coords[self.mesh.elements]
